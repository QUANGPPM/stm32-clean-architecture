...
const Stm32Gpio Stm32Gpio::none = Stm32Gpio(nullptr, 0); 
...
bool Stm32Gpio::subscribe(bool rising_edge, bool falling_edge, void (*callback)(void*), void* ctx) {
    ...
    if (!__atomic_compare_exchange_n(&subscription.port, &no_port, port, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
    ...
    temp |= ((uint32_t)(GPIO_GET_INDEX(port)) << (4U * (pin_number & 0x03U)));
    SYSCFG->EXTICR[pin_number >> 2U] = temp;


    if (rising_edge) {
        EXTI->RTSR |= (uint32_t)pin_mask;
    } else {
        EXTI->RTSR &= ~((uint32_t)pin_mask);
    }

    if (falling_edge) {
        EXTI->FTSR |= (uint32_t)pin_mask;
    } else {
        EXTI->FTSR &= ~((uint32_t)pin_mask);
    }

    EXTI->EMR &= ~((uint32_t)pin_mask);
    EXTI->IMR |= (uint32_t)pin_mask;
